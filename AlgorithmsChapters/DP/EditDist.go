...
func EditDist(str1 string, str2 string) int {
	m := len(str1)
	n := len(str2)
	return editDistUtil(str1, str2, m, n)
}

func editDistUtil(str1 string, str2 string, m int, n int) int {
	...
	return 1 + min(editDistUtil(str1, str2, m, n-1), // Insert
		editDistUtil(str1, str2, m-1, n),   // Remove
		editDistUtil(str1, str2, m-1, n-1)) // Replace
}

func EditDistDP(str1 string, str2 string) int {
	m := len(str1)
	n := len(str2)
	dp := make([][]int, m+1)
	for i := range dp {
